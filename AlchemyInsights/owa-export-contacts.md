---
title: '8000020: Exportieren von Kontakten'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: 9eb1c6be8b13fca300d4affe2ba213dcbb3e3ecb
ms.sourcegitcommit: 49eaa1417714617d768df85fd79b65e35b6e5c83
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 02/11/2022
ms.locfileid: "62549847"
---
# <a name="export-contacts"></a>Exportieren von Kontakten

1. Wählen Sie unten auf der Seite in Outlook im Web das <img src='data:image/png;base64,' />
 **Personen**.
2. Wählen Sie auf der Symbolleiste **"ManageExport"** > -**Kontakte aus**. 
3. Wählen Sie aus, ob alle Kontakte oder nur Kontakte aus einem bestimmten Ordner exportiert werden sollen, und wählen Sie dann " **Exportieren**" aus.
4. Wählen Sie unten auf der Seite " **Speichern** " aus, um "contacts.csv" im Standardordner "Downloads" zu speichern. Oder wählen Sie den Pfeil neben **"Speichern"** aus, und wählen Sie " **Speichern unter" aus**.

Weitere Informationen finden Sie unter [Exportieren von Kontakten aus Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

